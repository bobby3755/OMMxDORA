...
def avt_filter(*inputs):
    ...
    if rad_filter_type_upper == "zscore":
        up_fil = my_zscore < z_up  # Create a boolean filter for questionable zscores that are too high
    else:
        # if we are not using zscore, we use distance. Gather all the upper questionable distances
        up_fil = my_rad < dist_high

    # Filter for good data: Lower BOUND
    if rad_filter_type_lower == "zscore":
        # Gather lower questionable z scores, if we are talking about zscores.
        down_fil = z_down < my_zscore
    else:
        # Otherwise let's gather the lower distances that are questionable.
